...
- **`reinterpret_cast`:** For casts **without** well-defined semantics.
    ...
    - Ex. 
        ```C++
        Book *b = new Book { ... };
        vector<int> *v = reinterpret_cast<vector<int> *>(b);
        ```
...
- **`const_cast`:** for adding/removing const
    ...
    - Ex. 
        ```C++
        void g(Book &b);    // Suppose we know g won't change b, in the circumstances f would call it.
        void f(const Book &b) {
            g(const_cast<Book &>(b));
        }
        ```
...
- **`dynamic_cast`:**  `Book *pb = ...`
    ...
    - Dynamic reference casting:
        ...
        ```
    - **Note:** Dynamic casting works by accessing an object's **Run-Time Type Information (RTTI)**, this is stored in the vtable for the class.
    ...
    - Dynamic reference casting offers a possible solution to take polymorphic assignment problem:
        ...
        Text &Text::operator=(const Book &other) { // Virtual
            Text &textother = dynamic_cast<const Text &>(other);  // Throws if other is not a Text
